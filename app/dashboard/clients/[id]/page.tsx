...
// ─── Fils de discussion mail (chantier fiche client, 29.08.2026) ────────────
// Les échanges mail avec le client, regroupés en fils de conversation par le
// connecteur jardi-mail (route /api/clients/[id]/fils → /api/client-fils du
// connecteur : en-têtes References en composantes + sujet normalisé par
// client, dédup par message_id — même logique que le chat Jardi et
// client_dossier). Lecture seule : l'index Supabase, jamais IMAP, aucun flag.
type MessageFil = {
  date: string | null
...
function fmtDateHeureFil(iso: string | null) {
  if (!iso) return "—"
...
function FilsMailsBlock({ clientId }: { clientId: number }) {
  ...
  useEffect(() => {
    let actif = true
    fetch(`/api/clients/${clientId}/fils`)
      .then(async r => {
                ...
                <button type="button" onClick={() => basculer(i)}
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="mt-0.5 text-xs text-zinc-500">
                      {f.nb_messages} message{f.nb_messages > 1 ? "s" : ""} · {fmtDate(f.du)} → {fmtDate(f.au)}
                    </div>
                  </div>
                      ...
                      {f.messages.map((m, j) => (
                        <div key={j}
                          ...
                          <div className="flex flex-wrap items-center gap-2 text-xs text-zinc-400">
                            ...
                            <span>·</span>
                            <span>{fmtDateHeureFil(m.date)}</span>
                            {m.boite && (
...
{/* COMMANDES SHOPIFY */}
        ...
        {/* FILS DE DISCUSSION MAIL (chantier fiche client, 29.08.2026) */}
        <FilsMailsBlock clientId={client.id} />
